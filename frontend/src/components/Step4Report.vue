...
const toggleRawResult = (timestamp, event) => {
  ...
  // Wait for DOM update, then adjust scroll position to keep button at same position
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    if (queryMatch) result.query = (queryMatch[1] || queryMatch[2]).trim()

    // Extract prediction scenario
    const reqMatch = text.match(/Prediction Scenario:\s*(.+?)(?:\n|$)|预测场景:\s*(.+?)(?:\n|$)/)
    if (reqMatch) result.simulationRequirement = (reqMatch[1] || reqMatch[2]).trim()

    // Extract statistics - match "Related Prediction Facts: X"
    const factMatch = text.match(/(?:Related Prediction Facts|相关预测事实):\s*(\d+)/)
    const entityMatch = text.match(/(?:Related Entities|涉及实体):\s*(\d+)/)
    const relMatch = text.match(/(?:Relationship Chain|关系链):\s*(\d+)/)
    if (factMatch) result.stats.facts = parseInt(factMatch[1])
    if (entityMatch) result.stats.entities = parseInt(entityMatch[1])
    if (relMatch) result.stats.relationships = parseInt(relMatch[1])

    // Extract sub-questions - full extraction
    const subQSection = text.match(/###\s*(?:Analysis Sub-Questions|分析的子问题)\n([\s\S]*?)(?=\n###|$)/)
    if (subQSection) {
      const lines = subQSection[1].split('\n').filter(l => l.match(/^\d+\./))
      result.subQueries = lines.map(l => l.replace(/^\d+\.\s*/, '').trim()).filter(Boolean)
    }

    // Extract key facts - full extraction
    ...
    // Extract relationship chain - full extraction
    const relSection = text.match(/###\s*(?:【Relationship Chain】|【关系链】)\n([\s\S]*?)(?=\n###|$)/)
    if (relSection) {
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract active facts - full extraction
    ...
    return () => h('div', { class: 'insight-display' }, [
      ...
      h('div', { class: 'insight-tabs' }, [
        ...
        h('button', {
          ...
          h('span', { class: 'tab-label' }, `Key Entities (${props.result.entities.length})`)
        ]),
        h('button', {
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'entities' && props.result.entities.length > 0 && h('div', { class: 'entities-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, 'Key Entities'),
        ...
        activeTab.value === 'entities' && props.result.entities.length === 0 && h('div', { class: 'empty-state' }, 'No key entities yet'),
...
.header-dot {
  ...
  box-shadow: 0 0 0 3px rgba(var(--accent-rgb, 251, 146, 60), 0.2);
  margin-right: 10px;
  flex-shrink: 0;
...
@keyframes pulse-dot {
  ...
  50% {
    box-shadow: 0 0 0 5px rgba(var(--accent-rgb, 251, 146, 60), 0.1);
  }
}
...
.right-panel {
  ...
  --wf-active-bg: var(--bg-elevated);
  --wf-active-border: var(--accent);
  --wf-active-dot: var(--accent);
  --wf-active-text: var(--text-primary);

  --wf-done-bg: var(--bg-card);
  --wf-done-border: var(--border-subtle);
  --wf-done-dot: var(--status-success);
